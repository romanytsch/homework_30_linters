...
* Научиться добавлять визуальную составляющую на сайт с помощью шаблонов и передавать 
в них необходимые данные.
...
Подключите библиотеки `wtforms` и `flask_wtf`, которые мы использовали в модуле 4, 
и с помощью них добавьте валидацию — поля должны быть заполнены.
...
В практике №2 мы добавили endpoint, который принимает на вход имя автора, а возвращает все 
его книги.
...
* Необходимые переменные передаются в шаблон с помощью именованных параметров 
в `render_template`.
